...
describe('SecureExpressionEvaluator', () => {
  ...
  it('evaluates boolean and comparison expressions', () => {
    const res = SecureExpressionEvaluator.evaluate(
      'ctx.value > 5 && ctx.enabled',
      {
        value: 10,
        enabled: true,
      },
  ...
  it('evaluates arrays and nested expressions', () => {
    const res = SecureExpressionEvaluator.evaluate('[ctx.a, ctx.b + 1, true]', {
      a: 1,
      b: 2,
    });
  ...
  it('evaluates object literals with ctx references', () => {
    const res = SecureExpressionEvaluator.evaluate('{a: ctx.a, b: ctx.b}', {
      a: 5,
      b: 6,
    });
