...
describe('🎯 Fractional Sizing Utilities', () => {
  ...
  describe('Fractions with Variants', () => {
    ...
    it('should work with container query variants', () => {
      ...
      expect(css).toContain(
        '@container (min-width:24rem){.\\@sm\\:w-1\\/2{width:50%;}',
      );
      expect(css).toContain(
        '@container (min-width:28rem){.\\@md\\:w-2\\/3{width:66.66666666666666%;}',
      );
      expect(css).toContain(
        '@container (min-width:32rem){.\\@lg\\:h-3\\/4{height:75%;}',
      );
    });
    ...
    it('should work with hover and focus variants', () => {
      ...
      expect(css).toContain(
        '.focus\\:w-2\\/3:focus{width:66.66666666666666%;}',
      );
    });
    ...
    it('should work with dark mode variant', () => {
      ...
      expect(css).toContain(
        '@media (prefers-color-scheme: dark){.dark\\:w-1\\/2{width:50%;}',
      );
      expect(css).toContain(
        '@media (prefers-color-scheme: dark){.dark\\:h-3\\/4{height:75%;}',
      );
    });
  });
