...
describe('Transitions - Comprehensive Tests', () => {
  ...
  it('should properly map old node keys to new node keys', async () => {
    ...
    const items = Array.from(
      shadow.querySelectorAll('[data-test-id]'),
    ) as HTMLElement[];
    ...
    items.forEach((item) => {
      ...
      console.log(
        `  ID: ${item.getAttribute('data-test-id')}, key property: ${key}, data-anchor-key: ${dataKey}`,
      );
    });

    // Shuffle
    const shuffleBtn = shadow.querySelector(
      '[data-test="shuffle"]',
    ) as HTMLButtonElement;
    shuffleBtn.click();
    ...
    itemsAfter.forEach((item) => {
      ...
      console.log(
        `  ID: ${testId}, key property: ${key}, data-anchor-key: ${dataKey}`,
      );
  ...
  it('should handle rapid adds and removes correctly', async () => {
    ...
    console.log(
      `Items with transition classes: ${hasTransitionClasses} / ${items?.length}`,
    );
