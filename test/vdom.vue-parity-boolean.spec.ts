...
describe('vdom Vue-parity: boolean and null/undefined handling matrix', () => {
  it('input[type=text] value null/undefined -> empty string, disabled true/false behavior', () => {
    ...
    let tree = vnode('input', undefined, undefined, {
      attrs: { type: 'text', value: 'hello', disabled: true },
    });
    ...
    tree = vnode('input', undefined, undefined, {
      attrs: { type: 'text', value: null, disabled: false },
    });
    vdomRenderer(root, tree);
  ...
  it('checkbox checked true/false and value handling', () => {
    const root = document.createElement('div').attachShadow({ mode: 'open' });
    let tree = vnode('input', undefined, undefined, {
      attrs: { type: 'checkbox', checked: true, value: 'x' },
    });
    vdomRenderer(root, tree);
    ...
    tree = vnode('input', undefined, undefined, {
      attrs: { type: 'checkbox', checked: false, value: 'x' },
    });
    vdomRenderer(root, tree);
  ...
  it('textarea value null/undefined -> empty string', () => {
    const root = document.createElement('div').attachShadow({ mode: 'open' });
    let tree = vnode('textarea', 'ignored', undefined, {
      attrs: { value: 'ta' },
    });
    vdomRenderer(root, tree);
  ...
  it('select and option selected/value behavior', () => {
    ...
    let tree = vnode('select', [optionA, optionB], undefined, {
      attrs: { value: 'b' },
    });
    vdomRenderer(root, tree);
    ...
    tree = vnode('select', [optionA, optionB], undefined, {
      attrs: { value: null },
    });
    vdomRenderer(root, tree);
  ...
  it('progress value property assignment and null handling', () => {
    const root = document.createElement('div').attachShadow({ mode: 'open' });
    let tree = vnode('progress', undefined, undefined, {
      attrs: { value: 42 },
    });
    vdomRenderer(root, tree);
    ...
    expect(
      pr.value === 42 || pr.value === 1 || pr.getAttribute('value') === '42',
    ).toBe(true);
