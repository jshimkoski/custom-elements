...
describe(':model integration', () => {
  it('native input :model should update reactive ref', async () => {
    component('native-model-test', () => {
      const value = ref('');
      return html`<input :model="${value}" data-testid="inp" />
        <span data-testid="out">${value.value}</span>`;
    });
    ...
    const out = el.shadowRoot?.querySelector(
      '[data-testid="out"]',
    ) as HTMLElement;
  ...
  it('custom element :model:prop should update nested object via ReactiveState', async () => {
    component('child-model-prop', () => {
      ...
      return html`<button
        ...
        @click="${() => {
          ...
          try {
            console.log(
              '[test-child] click handler sees props.data.name =',
              props.data.name,
            );
          } catch {
            void 0;
          ...
          try {
            console.log('[test-child] emitting update:name ->', next);
          } catch {
            void 0;
          }
    ...
    component('parent-model-prop', () => {
      ...
      return html`${state.value.name}<child-model-prop
          :model:data="${state}"
        />`;
    });
    ...
    try {
      console.log('[test] child own keys=', Object.keys(child as any));
    } catch {
      void 0;
    }
    try {
      console.log('[test] child.data (prop) =', (child as any).data);
    } catch {
      void 0;
    }
    try {
      console.log('[test] child.context =', (child as any).context);
    } catch {
      void 0;
    }
    try {
      console.log(
        '[test] child.context.data =',
        (child as any).context && (child as any).context.data,
      );
    } catch {
      void 0;
    }
