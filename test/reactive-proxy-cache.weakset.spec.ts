...
describe('ReactiveProxyCache WeakSet behavior', () => {
  ...
  it('returns same proxy for same target and caches by target', () => {
    ...
    const p1 = ProxyOptimizer.createReactiveProxy(target, onUpdate, makeReactive);
    const p2 = ProxyOptimizer.createReactiveProxy(target, onUpdate, makeReactive);
  ...
  it('passing a proxy returns the proxy instance directly', () => {
    ...
    const p = ProxyOptimizer.createReactiveProxy(target, onUpdate, makeReactive);
  ...
  it('markAsProxy tracks the proxy instance (indirectly observable)', () => {
    ...
    const proxy = ProxyOptimizer.createReactiveProxy(target, onUpdate, makeReactive);
    ...
    const returned = ProxyOptimizer.createReactiveProxy(proxy as any, onUpdate, makeReactive);
