...
describe('Boolean Props', () => {
  it('should treat standalone attributes as boolean true', async () => {
    ...
    component(testTag, () => {
      const { disabled, checked } = useProps({
        disabled: false,
        checked: false,
      });
    ...
    expect(shadowRoot?.querySelector('#disabled-value')?.textContent).toBe(
      'true',
    );
    expect(shadowRoot?.querySelector('#checked-value')?.textContent).toBe(
      'true',
    );
  ...
  it('should handle mixed boolean and string attributes', async () => {
    ...
    expect(shadowRoot?.querySelector('#title-value')?.textContent).toBe(
      'Test Title',
    );
    expect(shadowRoot?.querySelector('#active-value')?.textContent).toBe(
      'false',
    );
