...
describe('compiler: preserve class-like attributes for custom elements', () => {
  ...
  it('preserves class and class-like attributes on host element', async () => {
    component('preserve-test', () => {
      return html`<div><child-el class="foo bar" activeClass="bg-red-500" custom-class="baz"></child-el></div>`;
    });
  ...
  const activeAttr = host!.getAttribute('activeClass') || host!.getAttribute('activeclass');
  ...
  const customVal = customKebab ?? (typeof customProp === 'string' ? customProp : null);
