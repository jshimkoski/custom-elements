...
describe("🎯 Container Query Precedence", () => {
  ...
  it("should order all named container query breakpoints correctly", () => {
    ...
    const positions = [
      { name: "xs", query: "@container (min-width:20rem)", index: css.indexOf("@container (min-width:20rem)") },
      { name: "sm", query: "@container (min-width:24rem)", index: css.indexOf("@container (min-width:24rem)") },
      { name: "md", query: "@container (min-width:28rem)", index: css.indexOf("@container (min-width:28rem)") },
      { name: "2xl", query: "@container (min-width:42rem)", index: css.indexOf("@container (min-width:42rem)") },
      { name: "5xl", query: "@container (min-width:64rem)", index: css.indexOf("@container (min-width:64rem)") },
      { name: "7xl", query: "@container (min-width:80rem)", index: css.indexOf("@container (min-width:80rem)") },
    ];
  ...
  it("should sort arbitrary queries between semantic breakpoints", () => {
    ...
    const positions = [
      { name: "xs (320px)", index: css.indexOf("@container (min-width:20rem)") },
      { name: "[350px]", index: css.indexOf("@container (min-width:350px)") },
      { name: "sm (384px)", index: css.indexOf("@container (min-width:24rem)") },
      { name: "[420px]", index: css.indexOf("@container (min-width:420px)") },
      { name: "md (448px)", index: css.indexOf("@container (min-width:28rem)") },
      { name: "[500px]", index: css.indexOf("@container (min-width:500px)") },
      { name: "lg (512px)", index: css.indexOf("@container (min-width:32rem)") },
      { name: "[550px]", index: css.indexOf("@container (min-width:550px)") },
      { name: "xl (576px)", index: css.indexOf("@container (min-width:36rem)") },
    ];
    ...
    // Verify they are in ascending order
    for (let i = 1; i < positions.length; i++) {
      expect(positions[i - 1].index, `${positions[i - 1].name} should come before ${positions[i].name}`).toBeLessThan(positions[i].index);
    }
  });
  ...
  it("should sort arbitrary queries with colors alongside semantic breakpoints", () => {
    ...
    for (let i = 1; i < positions.length; i++) {
      expect(positions[i - 1].index, `${positions[i - 1].name} < ${positions[i].name}`).toBeLessThan(positions[i].index);
    }
  });
  ...
  it("should sort many mixed arbitrary and semantic queries", () => {
    ...
    const positions = [
      { name: "xs (320px)", size: 320, index: css.indexOf("@container (min-width:20rem)") },
      { name: "[340px]", size: 340, index: css.indexOf("@container (min-width:340px)") },
      { name: "sm (384px)", size: 384, index: css.indexOf("@container (min-width:24rem)") },
      { name: "[480px]", size: 480, index: css.indexOf("@container (min-width:480px)") },
      { name: "xl (576px)", size: 576, index: css.indexOf("@container (min-width:36rem)") },
      { name: "[650px]", size: 650, index: css.indexOf("@container (min-width:650px)") },
      { name: "3xl (768px)", size: 768, index: css.indexOf("@container (min-width:48rem)") },
      { name: "[1100px]", size: 1100, index: css.indexOf("@container (min-width:1100px)") },
      { name: "7xl (1280px)", size: 1280, index: css.indexOf("@container (min-width:80rem)") },
    ];
    ...
    for (let i = 1; i < positions.length; i++) {
      expect(positions[i - 1].index, `${positions[i - 1].name} (${positions[i - 1].size}px) < ${positions[i].name} (${positions[i].size}px)`).toBeLessThan(positions[i].index);
    }
  });
  ...
  it("should order dark mode with container queries correctly", () => {
    ...
    const darkSmIndex = css.indexOf("@media (prefers-color-scheme: dark)@container (min-width:24rem)");
    const darkLgIndex = css.indexOf("@media (prefers-color-scheme: dark)@container (min-width:32rem)");
