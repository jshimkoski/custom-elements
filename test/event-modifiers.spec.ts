...
describe('🎯 Event Modifiers', () => {
  ...
  it('should handle .stop modifier to stopPropagation', async () => {
    ...
    component('test-stop', () => html`
      <div @click="${() => { outerEventReceived = true; }}">
        <button @click.stop="${() => { innerEventReceived = true; }}">Click me</button>
      </div>
    `);
  ...
  it('should handle .once modifier for single-use event listeners', async () => {
    ...
    component('test-once', () => html`
      <button @click.once="${() => { clickCount++; }}">Click me once</button>
    `);
  ...
  it('should handle combined modifiers', async () => {
    ...
    component('test-combined', () => html`
      <div @click="${() => { propagationStopped = false; }}">
        <form @submit.prevent.stop="${(e: Event) => {
          eventReceived = true;
          wasDefaultPrevented = e.defaultPrevented;
    ...
    const form = container.querySelector('test-combined')?.shadowRoot?.querySelector('form');
