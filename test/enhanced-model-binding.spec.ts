  ...
  it('should support reactive state with checkbox inputs', async () => {
    component('test-checkbox-model', () => {
      ...
      return html`
        ...
        <span data-testid="checkbox-display"
          >${checked.value ? 'checked' : 'unchecked'}</span
        >
      `;
    });
  ...
  it('should work with both reactive state objects and direct values', async () => {
    ...
    component('test-mixed-model', () => {
      ...
      return html`
        ...
        <input
          value="${reactiveInput.value}"
          @input="${(e: Event) =>
            (reactiveInput.value = (e.target as HTMLInputElement).value)}"
    ...
    const shadowRoot = testEl.shadowRoot;
    const reactiveInput = shadowRoot?.querySelector(
      '[data-testid="reactive-input"]',
    ) as HTMLInputElement;
    const manualInput = shadowRoot?.querySelector(
      '[data-testid="manual-input"]',
    ) as HTMLInputElement;
