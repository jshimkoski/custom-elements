...
describe('Runtime V2 Form Bindings', () => {
  ...
  describe('v-model basic functionality', () => {
    ...
    it('should handle combined modifiers v-model.trim.number', async () => {
      const ElementClass = createElementClass({
        state: { value: 0 },
        render: (state) => html`<input type="text" v-model="value.trim.number" />`,
        style: '',
      });
  ...
  describe('checkbox bindings', () => {
    ...
    it('should handle single checkbox with custom true/false values', async () => {
      const ElementClass = createElementClass({
        state: { status: 'no' },
        render: (state) => html`<input type="checkbox" v-model="status" true-value="yes" false-value="no" />`,
        style: '',
      });
  ...
  describe('nested property bindings', () => {
    it('should handle nested object properties', async () => {
      const ElementClass = createElementClass({
        ...
        render: (state) => html`<input type="text" v-model="user.profile.name" />`,
        style: '',
      });
  ...
  describe('backward compatibility', () => {
    it('should still work with data-bind attribute', async () => {
      const ElementClass = createElementClass({
        state: { message: 'test' },
        render: (state) => html`<input type="text" data-bind="message" />`,
        style: '',
      });

      customElements.define('data-bind-compat-test', ElementClass);

      const el = document.createElement('data-bind-compat-test') as any;
      container.appendChild(el);
      ...
      await new Promise(resolve => setTimeout(resolve, 10));
      expect(el._state.message).toBe('updated');
    });

    it('should work with data-model attribute with pipe modifiers', async () => {
      const ElementClass = createElementClass({
        state: { value: 0 },
        render: (state) => html`<input type="text" data-model="value|trim|number" />`,
        style: '',
      });

      customElements.define('data-model-pipe-test', ElementClass);

      const el = document.createElement('data-model-pipe-test') as any;
      container.appendChild(el);
      ...
      await new Promise(resolve => setTimeout(resolve, 10));
      expect(el._state.value).toBe(123);
    });
  });
});
