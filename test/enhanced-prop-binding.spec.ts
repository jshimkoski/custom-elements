...
describe('🔗 Enhanced :prop Binding', () => {
  ...
  it('should bind reactive state objects to native element properties with :prop', async () => {
    ...
    const display = shadowRoot!.querySelector(
      '[data-testid="display"]',
    ) as HTMLElement;
  ...
  it('should work with :model:prop bindings for reactive state objects', async () => {
    component('model-prop-test', () => {
      ...
      return html`
        ...
        <div data-testid="output">
          ${formData.value.name} - ${formData.value.age}
        </div>
        <button
          @click="${() => {
            formData.value = { name: 'Jane', age: 30 };
          }}"
          data-testid="update-btn"
        >
          Update
    ...
    const nameInput = shadowRoot!.querySelector(
      '[data-testid="name-input"]',
    ) as HTMLInputElement;
    const ageInput = shadowRoot!.querySelector(
      '[data-testid="age-input"]',
    ) as HTMLInputElement;
  ...
  it('should handle reactive state objects in data attributes', async () => {
    ...
    const div = shadowRoot!.querySelector(
      '[data-testid="test-div"]',
    ) as HTMLElement;
    const button = shadowRoot!.querySelector(
      '[data-testid="update-btn"]',
  ...
  it('should handle complex reactive state objects with nested properties', async () => {
    component('complex-props-test', () => {
      ...
      return html`
        ...
        >
          ${complexState.value.user.name} uses
          ${complexState.value.user.settings.theme} theme
        </div>
        <button
          @click="${() => {
            complexState.value = {
  ...
  it('should work with boolean reactive state properties', async () => {
    component('boolean-props-test', () => {
      ...
      return html`
        <input type="text" :disabled="${isDisabled}" data-testid="text-input" />
        <input
          type="checkbox"
          :checked="${isChecked}"
    ...
    const checkboxInput = shadowRoot!.querySelector(
      '[data-testid="checkbox-input"]',
    ) as HTMLInputElement;
    const button = shadowRoot!.querySelector(
      '[data-testid="toggle-btn"]',
