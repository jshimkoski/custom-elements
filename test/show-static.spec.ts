...
describe('test :show directive static', () => {
  ...
  it('should set display: none when :show is false', async () => {
    component('test-show-static', () => {
      ...
      return html` <div :show="${isVisible.value}">Should be hidden</div> `;
    });
    ...
    const div = container
      .querySelector('test-show-static')
  ...
  it('should not set display style when :show is true', async () => {
    component('test-show-static-visible', () => {
      ...
      return html` <div :show="${isVisible.value}">Should be visible</div> `;
    });

    container.innerHTML =
      '<test-show-static-visible></test-show-static-visible>';
    await new Promise((resolve) => setTimeout(resolve, 10));

    const div = container
      .querySelector('test-show-static-visible')
      ?.shadowRoot?.querySelector('div');
