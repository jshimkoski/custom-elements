...
describe('List Animation - Styling and Animation Tests', () => {
  ...
  it('should apply enter transitions when adding new items', async () => {
    component('test-list-add-items', () => {
      ...
      return html`
        <div>
          <button @click="${() => {
            items.value = [...items.value, { id: items.value.length + 1, text: `Item ${items.value.length + 1}` }];
    ...
    component('test-list-remove-items', () => {
      ...
      return html`
        <div>
          ...
          }}" data-test="remove">Remove First</button>
          ${TransitionGroup({
  ...
  it.skip('should apply move transitions when shuffling items (TODO: implement move transitions)', async () => {
    ...
    const getOrder = () => {
      const items = el.shadowRoot.querySelectorAll('[data-test-id]');
      return Array.from(items).map((item: any) => item.getAttribute('data-test-id'));
    };
  ...
  it('should handle rapid add/remove operations without breaking styles', async () => {
    component('test-list-rapid-changes', () => {
      ...
      return html`
        <div>
          <button @click="${() => {
            items.value = [...items.value, { id: Date.now(), text: `Item ${items.value.length + 1}` }];
          }}" data-test="add">Add</button>
          <button @click="${() => {
  ...
  it('should preserve element identity during reorder (items move, not recreate)', async () => {
    component('test-style-preservation', () => {
      ...
      return html`
        <div>
          <button @click="${() => {
            items.value = [items.value[2], items.value[0], items.value[1]];
          }}" data-test="reorder">Reorder</button>
          ${TransitionGroup({
            preset: 'fade',
            tag: 'div'
          }, each(items.value, (item: any) => html`
    ...
    const getOrder = () => {
      const items = el.shadowRoot.querySelectorAll('[data-test-id]');
      return Array.from(items as NodeListOf<HTMLElement>).map(item => item.getAttribute('data-test-id'));
    };

    const beforeOrder = getOrder();
    expect(beforeOrder).toEqual(['1', '2', '3']);

    const reorderBtn = el.shadowRoot.querySelector('[data-test="reorder"]') as HTMLElement;
    reorderBtn.click();
  ...
  it('should not duplicate classes when patching existing elements', async () => {
    component('test-no-duplicate-classes', () => {
      ...
      return html`
        <div>
          ...
          }}" data-test="rerender">Re-render</button>
          ${TransitionGroup({
            preset: 'fade',
            tag: 'div'
          }, each(items.value, (item: any) => html`
    ...
    const rerenderBtn = el.shadowRoot.querySelector('[data-test="rerender"]') as HTMLElement;
