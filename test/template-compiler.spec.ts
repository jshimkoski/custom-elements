...
describe('template-compiler', () => {
  it('should handle deeply nested fragments', () => {
    const vnode = html`<div>
      ${[
        html`<span>${[html`<b>${'deep'}</b>`, html`<i>${'nest'}</i>`]}</span>`,
        html`<u>${'frag'}</u>`,
      ]}
    </div>`;
    ...
    if (Array.isArray(children)) {
      ...
      const span = children.find(
        (c) => typeof c === 'object' && c.tag === 'span',
      );
      expect(span).toBeDefined();
      if (span && Array.isArray(span.children)) {
        const b = span.children.find(
          (c) => typeof c === 'object' && c.tag === 'b',
        );
        const i = span.children.find(
          (c) => typeof c === 'object' && c.tag === 'i',
        );
  ...
  it('should handle malformed templates with missing closing tags', () => {
    ...
    if (Array.isArray(children)) {
      ...
      expect(span).toBeDefined();
      const div = children.find(
        (c) => typeof c === 'object' && c.tag === 'div',
      );
  ...
  it('should handle malformed templates with stray interpolation', () => {
    ...
    if (Array.isArray(vnode)) {
      ...
      expect(vnode.some((v) => typeof v === 'object' && v.tag === 'div')).toBe(
        true,
      );
    } else {
  ...
  it('should return fragment as array if multiple cleaned nodes', () => {
    ...
    if (Array.isArray(nodes)) {
      ...
      expect((nodes as VNode).tag === 'a' || (nodes as VNode).tag === 'b').toBe(
        true,
      );
    }
  });
  ...
  it('should deeply merge style and class attributes', () => {
    const vnode = html`<div style="color: red;" class="foo">
      ${{ attrs: { style: 'background: blue;', class: 'bar baz' } }}
    </div>`;
    expect(vnode).toBeDefined();
  ...
  it('should handle complex directive combinations and modifiers', () => {
    const vnode = html`<input
      :model.trim.number.lazy="foo.bar"
      :show="false"
      :class="active"
      :style="color: green;"
    />`;
    expect((vnode as VNode).props?.directives?.model.value).toBe('foo.bar');
    expect((vnode as VNode).props?.directives?.model.modifiers).toEqual(
      expect.arrayContaining(['trim', 'number', 'lazy']),
    );
    ...
    expect(['', 'color: green']).toContain(
      (vnode as VNode).props?.attrs?.style ?? '',
    );
    expect(['', 'active']).toContain(
      (vnode as VNode).props?.attrs?.class ?? '',
    );
    // :show false should add display: none
    // Accept undefined or string for style when checking display: none
    expect(['', 'display: none']).toContain(
      (vnode as VNode).props?.attrs?.style ?? '',
    );
  });
  ...
  it('should handle unusual attribute and directive combinations', () => {
    ...
    expect(typeof (vnode as VNode).props?.directives?.bind?.value).toBe(
      'string',
    );
    expect((vnode as VNode).props?.directives?.bind?.value).toContain(
      'disabled',
    );
    expect((vnode as VNode).props?.directives?.bind?.value).toContain('foo');
    expect((vnode as VNode).props?.attrs?.style ?? '').not.toContain(
      'display: none',
    );
  });
  ...
  it('should handle fragments (multiple root nodes)', () => {
    ...
    if (Array.isArray(nodes)) {
      ...
      if (nodes.tag === '#anchor' && Array.isArray(nodes.children)) {
        ...
        (nodes as VNode).tag === 'div' &&
        (nodes as VNode).children === ''
      ) {
        // Fallback empty div
  ...
  it('should parse directives', () => {
    ...
    expect((vnode as VNode).props?.directives?.model.modifiers).toContain(
      'lazy',
    );
  });
