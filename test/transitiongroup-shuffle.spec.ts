...
describe('TransitionGroup shuffle animations', () => {
  ...
  it('should apply move transitions when shuffling flex layout items', async () => {
    component('test-flex-shuffle', () => {
      ...
      return html`
        <div>
          <button @click="${() => {
            const shuffled = [items.value[3], items.value[0], items.value[2], items.value[1]];
    ...
    expect(items.map((item: any) => item.textContent?.trim())).toEqual(['A', 'B', 'C', 'D']);
    ...
    expect(items.map((item: any) => item.getAttribute('data-test-id'))).toEqual(['4', '1', '3', '2']);
    expect(items.map((item: any) => item.textContent?.trim())).toEqual(['D', 'A', 'C', 'B']);
  ...
  it('should apply move transitions when shuffling grid layout items', async () => {
    ...
    // Check initial order
    let items = Array.from(wrapper.querySelectorAll('[data-test-id]'));
    expect(items.map((item: any) => item.getAttribute('data-test-id'))).toEqual(['1', '2', '3', '4', '5', '6']);
    ...
    items = Array.from(wrapper.querySelectorAll('[data-test-id]'));
    expect(items.map((item: any) => item.getAttribute('data-test-id'))).toEqual(['6', '5', '4', '3', '2', '1']);
  ...
  it('should handle multiple rapid shuffles smoothly', async () => {
    ...
    const ids = Array.from(items).map((item: any) => item.getAttribute('data-test-id'));
