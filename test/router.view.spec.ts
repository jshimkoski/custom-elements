...
function mountAndGetHtml() {
  ...
  return new Promise<string>((resolve) => setTimeout(() => resolve(el.shadowRoot!.innerHTML), 0));
}
...
describe('router-view render types', () => {
  ...
  const routes = [{ path: '/home', component: () => ({ tag: 'div', props: {}, children: 'home' }) }];
  ...
  it('renders sync function component from load()', async () => {
    const routes = [
      {
        path: '/sync',
        load: () => Promise.resolve({ default: () => ({ tag: 'div', props: {}, children: 'sync' }) }),
      },
    ];
    ...
    const routes = [
      {
        ...
        load: async () => {
          ...
          return { default: async () => ({ tag: 'div', props: {}, children: 'async' }) };
        },
      },
    ];
