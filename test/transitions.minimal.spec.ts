...
import { component, html, ref, Transition, TransitionGroup, each } from "../src/lib/index";
...
describe("Transitions - Minimal Tests", () => {
  ...
  it("should trigger component re-render when ref changes", async () => {
      ...
      return html`
        <div data-test="container">
          ...
          ${TransitionGroup({
            ...
            <div key="${item.id}" data-test-id="${item.id}" data-test="list-item">${item.text}</div>
          `))}
        </div>
      `;
    });
    ...
    // Get initial order - items are now direct children of TransitionGroup div
    const itemsBefore = Array.from(el.shadowRoot?.querySelectorAll('[data-test-id]') || [])
      .map(el => el.getAttribute('data-test-id'));
    
    // Click shuffle button
    const btn = el.shadowRoot?.querySelector('[data-test="shuffle-btn"]') as HTMLButtonElement;
    btn.click();
    
    await new Promise(resolve => setTimeout(resolve, 100));

    // Get order after shuffle - items are now direct children of TransitionGroup div
    const itemsAfter = Array.from(el.shadowRoot?.querySelectorAll('[data-test-id]') || [])
      .map(el => el.getAttribute('data-test-id'));
  ...
  it("should show elements with enter transitions", async () => {
    component('test-transition-enter', () => {
      ...
      (window as any).toggleShow = () => { show.value = !show.value; };
      ...
      return html`
        <div>
          ${Transition({
            ...
            <div data-test="content">Hello</div>
          `)}
        </div>
      `;
    });
    ...
    let content = el.shadowRoot?.querySelector('[data-test="content"]') as HTMLElement;
    ...
    content = el.shadowRoot?.querySelector('[data-test="content"]') as HTMLElement;
  ...
  it("should apply leave transitions when removing elements", async () => {
        ...
        <div>
          ${TransitionGroup({
            ...
            <div key="${item.id}" data-test-id="${item.id}">
              ${item.text}
              <button @click="${() => remove(item.id)}" data-test-remove="${item.id}">Remove</button>
            </div>
          `))}
        </div>
      `;
    ...
    const removeBtn = el.shadowRoot?.querySelector('[data-test-remove="1"]') as HTMLButtonElement;
    removeBtn.click();
