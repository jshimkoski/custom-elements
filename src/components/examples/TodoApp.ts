 ...
 */
import { component, html, each, type ComponentContext } from '../../lib';
interface Todo {
  id: number;
  text: string;
  done: boolean;
...
type TodoAppContext = ComponentContext<State, {}, {}, Methods>;

type Methods = {
  submitForm(event: SubmitEvent, ctx: TodoAppContext): void;
  addTodo(ctx: TodoAppContext): void;
  toggleTodo(id: number, ctx: TodoAppContext): void;
  removeTodo(id: number, ctx: TodoAppContext): void;
};

export const TodoApp = component<State, {}, {}, Methods>('todo-app', (ctx) => html`
    ...
    <ul>
      ${each(ctx.todos, (todo) => html`
        <li class="flex items-center gap-2 py-2 border-b border-neutral-200 dark:border-neutral-800">
          <input
            type="checkbox"
            :checked="${todo.done}" @change="${() => ctx.toggleTodo(todo.id)}"
          >
          <span
            :class="${{
              'grow text-left ': true,
