...
import { routeComponent, createRouteTemplate, type VNode } from "./runtime";

export interface Route {
  path: string;
  component?: string;
  load?: () => Promise<{ default: string | HTMLElement | Function }>;
  asyncComponent?: string; // For pre-registered async components
}
...
export async function resolveRouteComponent(route: Route): Promise<any> {
  ...
  if (route.asyncComponent) return route.asyncComponent;
  if (route.load) {
...
/**
 * Helper to create async route components
 */
export function createAsyncRoute<S extends object = {}>(
  path: string,
  loader: () => Promise<{ default: string | Function }>,
  options: {
    tagName?: string;
    state?: S;
    loadingTemplate?: (state: S) => VNode | VNode[];
    errorTemplate?: (error: Error, state: S) => VNode | VNode[];
  } = {},
): Route {
  const tagName =
    options.tagName || `route-${path.replace(/[^a-zA-Z0-9]/g, "-")}`;

  // Register the route component
  routeComponent(tagName, {
    routeLoader: loader,
  ...
  return {
    path,
    asyncComponent: tagName,
    load: loader,
  };
}

/**
 * Helper to create multiple async routes
 */
export function createAsyncRoutes(
  routes: Array<{
    ...
    tagName?: string;
    state?: object;
    loadingTemplate?: (state: any) => VNode | VNode[];
    errorTemplate?: (error: Error, state: any) => VNode | VNode[];
  }>,
): Route[] {
  return routes.map(({ path, loader, ...options }) =>
    createAsyncRoute(path, loader, options),
  );
}
