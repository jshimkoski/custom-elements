...
class EventManager {
  ...
  static removeListener(
    ...
    if (cleanups) {
      // Find and remove the first cleanup function for this event type
      // This is a limitation, but in practice most elements don't have
      // the same event type added multiple times
      const index = cleanups.findIndex(() => true);
      
      if (index >= 0) {
        cleanups.splice(index, 1);
        
        // If no more cleanups, remove the element from WeakMap
  ...
  static hasListeners(element: HTMLElement): boolean {
    const cleanups = this.cleanupFunctions.get(element);
    return cleanups !== undefined && cleanups.length > 0;
  }
