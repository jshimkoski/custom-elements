describe('Complete Model Binding Test - Vue.js Compatible', () => {
  beforeEach(() => {
    ...
    cy.get('switch-test').then(($el) => {
      const switchTestEl = $el[0] as any;
  ...
  it('should work exactly like Vue.js :model directive', () => {
    ...
    cy.get('switch-test').shadow().find('p').should('contain', 'Current value: false');
    cy.get('switch-test').shadow().find('cer-switch').shadow().find('input[type="checkbox"]').should('not.be.checked');
    cy.get('switch-test').shadow().find('simple-switch').shadow().find('input[type="checkbox"]').should('not.be.checked');

    // Test 1: Click cer-switch once to set to true
    cy.get('switch-test').shadow().find('cer-switch').shadow().find('label').click();
    
    // All components should immediately reflect true state
    cy.get('switch-test').shadow().find('p').should('contain', 'Current value: true');
    cy.get('switch-test').shadow().find('cer-switch').shadow().find('input[type="checkbox"]').should('be.checked');
    cy.get('switch-test').shadow().find('simple-switch').shadow().find('input[type="checkbox"]').should('be.checked');
    ...
    cy.get('switch-test').shadow().find('simple-switch').shadow().find('input[type="checkbox"]').click({ force: true });
