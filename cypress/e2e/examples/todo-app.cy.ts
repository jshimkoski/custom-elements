...
describe.skip('TodoApp Component', () => {
  ...
  it('adds multiple todo items', () => {
    cy.get('todo-app').shadow().within(() => {
      ...
      cy.get('input[type="text"]').should('exist');
      cy.get('button[type="submit"]').should('exist');
      cy.get('input[type="text"]').type('Buy milk').should('have.value', 'Buy milk');
      cy.get('button[type="submit"]').click();
      cy.get('input[type="text"]').should('not.have.text');
      cy.get('input[type="text"]').type('Walk dog').should('have.value', 'Walk dog');
      cy.get('button[type="submit"]').click();
      cy.get('input[type="text"]').should('not.have.text');
      cy.get('ul li').should('have.length', 2);
      cy.get('ul li').eq(0).find('span.todo-text').should('contain', 'Buy milk');
      cy.get('ul li').eq(1).find('span.todo-text').should('contain', 'Walk dog');
    });
  });
  ...
  it('marks a todo item as completed and toggles back', () => {
    cy.get('todo-app').shadow().within(() => {
      ...
      cy.get('ul li').eq(0).find('span.todo-text').should('have.attr', 'data-done', 'true');
      cy.get('ul li').eq(0).find('input[type="checkbox"]').uncheck();
      cy.get('ul li').eq(0).find('span.todo-text').should('have.attr', 'data-done', 'false');
    });
  });
  ...
  it('is accessible by keyboard', () => {
    cy.get('todo-app').shadow().within(() => {
      ...
      cy.get('ul li').eq(0).find('span.todo-text').should('contain', 'Do homework');
    });
  });
