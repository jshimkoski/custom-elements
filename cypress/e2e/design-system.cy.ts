  ...
  describe('📝 Text Input Components', () => {
    ...
    it('should handle ds-input :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.contains('Value: "Updated Text"', { timeout: 10000 }).should('exist');

        // Verify second ds-input (bind) shows the same value
        cy.get('ds-input').eq(1).shadow().within(() => {
          cy.get('input', { timeout: 10000 }).should('have.value', 'Updated Text');
        });
      });
    });
    ...
    it('should handle ds-textarea :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.contains('Value: "This is a multi-line\\ntext area example"').should('exist');
        
        cy.get('ds-textarea').first().shadow().within(() => {
  ...
  describe('✅ Boolean Input Components', () => {
    ...
    it('should handle ds-checkbox :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-checkbox').first().shadow().within(() => {
  ...
  describe('🎯 Selection Components', () => {
    ...
    it('should handle ds-select :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-select').first().shadow().within(() => {
        ...
        cy.get('ds-select').eq(1).shadow().within(() => {
  ...
  describe('🔢 Numeric Input Components', () => {
    ...
    it('should handle ds-number :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-number').first().shadow().within(() => {
    ...
    it('should handle ds-range :model binding correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-range').first().shadow().within(() => {
    ...
    it('should display ds-progress value correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-progress').first().shadow().within(() => {
  ...
  describe('🎬 Action Components', () => {
    ...
    it('should handle ds-button click events correctly', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-button').contains('Reset Text Inputs').click({ force: true });
        cy.contains('Last Action: Text Reset').should('exist');
        
        cy.get('ds-button').contains('Toggle Booleans').click({ force: true });
        cy.contains('Last Action: Boolean Toggle').should('exist');
        
        cy.get('ds-button').contains('Randomize Numbers').click({ force: true });
  ...
  describe('🔄 Cross-Component Integration Tests', () => {
    ...
    it('should validate :model vs :bind behavior differences', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        cy.get('ds-input').eq(1).shadow().within(() => {
          cy.get('input', { timeout: 10000 }).should('have.value', 'Model Updated');
        });
  ...
  describe('🏆 Final Validation', () => {
    ...
    it('should confirm all design system components work with :model, :bind, and :model:prop', () => {
      cy.get('design-system-test').shadow().within(() => {
        ...
        // Text Inputs
        cy.get('ds-input').first().shadow().find('input').clear().type('Final Test Input');
        cy.get('ds-textarea').first().shadow().find('textarea').clear().type('Final Test Textarea');
        
        // Boolean Inputs  
        cy.get('ds-checkbox').first().shadow().find('input[type="checkbox"]').uncheck();
        
        // Selection Inputs
        cy.get('ds-select').first().shadow().find('select').select('green');
        
        // Numeric Inputs
        cy.get('ds-number').first().shadow().find('input[type="number"]').clear().type('88');
        cy.get('ds-range').first().shadow().find('input[type="range"]').invoke('val', 33).trigger('input');
        ...
        cy.log('🎉 ALL DESIGN SYSTEM COMPONENTS WORKING WITH MODEL BINDING! 🎉');
      });
    });
  });
});
